...
def fetch_channel(ucid, pull_all_videos : Bool)
  ...
  # YouTube's per-channel RSS feed (/feeds/videos.xml?channel_id=) started
  # returning 404 in August 2026, which made every refresh raise "Deleted or
  # invalid channel" and get the channel marked deleted in the database.
  # Channel metadata and the video list now come from the same InnerTube
  # endpoints the channel pages already use, which still work.
  LOGGER.trace("fetch_channel: #{ucid} : Fetching channel metadata")
  about = get_about_info(ucid, nil)

  author = about.author
  auto_generated = about.auto_generated
  ...
  LOGGER.trace("fetch_channel: #{ucid} : Extracting videos from channel page")
